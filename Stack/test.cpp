...
bool test_empty_stack_operations(Stack* stack) {
    ...
    stack->clear();
    bool pop_hoynai= false;
    bool top_hoynai= false;
    // pop
    try {
        int val = stack->pop();
        if (val == -1) pop_hoynai =true; // as per implementation returns -1
    } catch (...) {
        pop_hoynai = true;
    }
    // top
    try {
        int val = stack->top();
        if (val == -1) top_hoynai =true;
    } catch (...) {
        top_hoynai =true;
    }
    return pop_hoynai && top_hoynai;
    //return true; // Placeholder
}
...
bool test_array_resizing(Stack* stack) {
    ...
    // Only ArrayStack resizes dynamically -_-
    ArrayStack* arr_stack = dynamic_cast<ArrayStack*>(stack);
    if (!arr_stack) return true; // skip for ListStack
    arr_stack->clear();
    int initial_cap = 10;
    // Push when >= 50% full
    ...
    if (arr_stack->size()!= initial_cap) return false;

    // Now pop when < 25% full
    for (int i = 0; i < initial_cap - 2; i++) {
        arr_stack->pop();
    }
    if (arr_stack->size()!= 2) return false;
...
bool test_stress(Stack* stack) {
    ...
    for (int i = N - 1; i >= 0; i--) {
        int valuuu = stack->pop();
        if (valuuu != i) return false;
    }
    return stack->empty();
    //return true; // Placeholder
}
